...
public class Biblioteca {
	...
	private CadastroEspacoEstudo cadastroEspEstudo;
	private CadastroLivros cadastroLivro;
	...
	public void pegarLivroEmprestado(Pessoa a, Livro l)
			throws EmprestimoException, LivroException {
		if (verificaAtributosEmprestimo(l, a)) {
			cadastroEmprestimo.cadastrar(new Emprestimo(a, l));
		}
	}

	public void devolverLivro(Pessoa a, Livro l) throws EmprestimoException,
			LivroException {
		if (verificaAtributosEmprestimo(l, a)) {
	...
	public void renovarEsp(Pessoa a, Livro l) throws EmprestimoException,
			LivroException {
		if (verificaAtributosEmprestimo(l, a)) {
	...
	public Emprestimo verificar(Emprestimo e) throws EmprestimoException,
			LivroException {
		Emprestimo emp = null;
		if (verificaAtributosEmprestimo(e.getLivro(), e.getAluno())) {
			emp = cadastroEmprestimo.procurar(e.getLivro(), e.getAluno());
		}
		return emp;
	}

	private boolean verificaAtributosEmprestimo(Livro l, Pessoa p)
			throws LivroException {
		cadastroLivro.procurar(l.getTitulo());
		if (cadastroPessoa.procurar(p) != null) {
			return true;
		}
		return false;
	}
	...
	public void reservarEspaco(int id, Aluno aluno)
